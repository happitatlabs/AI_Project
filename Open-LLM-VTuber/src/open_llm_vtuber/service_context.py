...
class ServiceContext:
    ...
    def init_live2d(self, live2d_model_name: str) -> None:
        ...
        try:
            ...
            logger.info(f"Live2D model initialized successfully: {live2d_model_name}")
            if self.live2d_model.model_info:
                logger.info(f"Model info URL: {self.live2d_model.model_info.get('url', 'N/A')}")
        except Exception as e:
            logger.critical(f"Error initializing Live2D: {e}")
            logger.critical(f"Model name: {live2d_model_name}")
            import traceback
    ...
    async def handle_config_switch(
        ...
        try:
            ...
            if new_character_config_data:
                ...
                model_info = None
                if self.live2d_model and hasattr(self.live2d_model, 'model_info'):
                    model_info = self.live2d_model.model_info
                
                if not model_info:
                    logger.error("[ServiceContext] Live2D model not initialized")
                    logger.error(f"[ServiceContext] live2d_model: {self.live2d_model}")
                    logger.error(f"[ServiceContext] live2d_model_name: {self.character_config.live2d_model_name if self.character_config else 'N/A'}")
