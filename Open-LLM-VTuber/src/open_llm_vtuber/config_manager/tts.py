...
class EdgeTTSConfig(I18nMixin):
    ...
    voice: str = Field(..., alias="voice")
    rate: Optional[str] = Field("+0%", alias="rate")  # Speech rate (e.g., "+15%", "-10%")
    pitch: Optional[str] = Field("+0Hz", alias="pitch")  # Pitch adjustment (e.g., "+2Hz", "-1Hz")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "rate": Description(
            en="Speech rate adjustment (e.g., '+15%' for faster, '-10%' for slower)",
            zh="语速调整（例如 '+15%' 更快，'-10%' 更慢）",
        ),
        "pitch": Description(
            en="Pitch adjustment (e.g., '+2Hz' for higher, '-1Hz' for lower)",
            zh="音调调整（例如 '+2Hz' 更高，'-1Hz' 更低）",
        ),
    }
