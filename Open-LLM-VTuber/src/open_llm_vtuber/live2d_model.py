...
class Live2dModel:
    ...
    def __init__(
        ...
        if not os.path.isabs(model_dict_path):
            # Try to find model_dict.json relative to the project root
            # Look for it in the current directory first, then try parent directories
            current_dir = Path.cwd()
            possible_paths = [
                current_dir / model_dict_path,
                Path(__file__).parent.parent.parent.parent / model_dict_path,
            ]
            for path in possible_paths:
                if path.exists():
                    model_dict_path = str(path.absolute())
                    break
            else:
                # If not found, use relative path (will fail with clear error)
                model_dict_path = str(current_dir / model_dict_path)
