...
class WebSocketHandler:
    ...
    def _init_message_handlers(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            "speak": self._handle_conversation_trigger,  # Direct TTS without LLM (handled in conversation_handler)
    ...
    async def _send_initial_messages(
        ...
        # Safely get model_info, handle case where live2d_model might be None
        model_info = None
        if session_service_context.live2d_model and hasattr(session_service_context.live2d_model, 'model_info'):
        ...
        if not model_info:
            ...
            logger.error(f"[WebSocket] live2d_model_name: {session_service_context.character_config.live2d_model_name if session_service_context.character_config else 'N/A'}")
            # Don't send None, send empty dict to prevent frontend errors
            model_info = {}
    ...
    async def _handle_conversation_trigger(
        ...
        msg_type = data.get("type", "")
        logger.debug(f"[WebSocket] Handling conversation trigger: type={msg_type}, client={client_uid}")
        
        # Safely get context, fallback to default if not found
        context = self.client_contexts.get(client_uid)
        if not context:
            logger.warning(f"[WebSocket] Client {client_uid} not found in contexts, using default context")
            context = self.default_context_cache
            # Add to contexts for future use
            self.client_contexts[client_uid] = context
        
        # Ensure received_data_buffers entry exists
        if client_uid not in self.received_data_buffers:
            self.received_data_buffers[client_uid] = np.array([])
        
        # Ensure current_conversation_tasks entry exists
        if client_uid not in self.current_conversation_tasks:
        ...
        if msg_type == "speak":
            ...
            async def broadcast_to_all_clients(message: str):
                """Broadcast message to all connected clients"""
                for uid, ws in self.client_connections.items():
                    try:
                        await ws.send_text(message)
                        logger.debug(f"[Broadcast] Sent to client {uid}")
                    except Exception as e:
                        logger.warning(f"[Broadcast] Failed to send to {uid}: {e}")

            # Use first browser client's context (not mellow_link's)
            browser_client_uid = None
            browser_context = context
            for uid in self.client_connections.keys():
                if uid != client_uid:  # Skip mellow_link's connection
                    browser_client_uid = uid
                    browser_context = self.client_contexts.get(uid, context)
                    break

            if browser_client_uid:
                logger.info(f"[WebSocket] Broadcasting speak to browser client: {browser_client_uid}")

            await handle_conversation_trigger(
                msg_type=msg_type,
                data=data,
                client_uid=browser_client_uid or client_uid,
                context=browser_context,
                websocket=websocket,  # Still use original for response
                client_contexts=self.client_contexts,
                client_connections=self.client_connections,
                chat_group_manager=self.chat_group_manager,
                ...
                current_conversation_tasks=self.current_conversation_tasks,
                broadcast_to_group=self.broadcast_to_group,
                websocket_send_override=broadcast_to_all_clients,  # NEW: override send function
            )
            return
    ...
    async def _handle_init_config_request(
        ...
        # Safely get model_info, handle case where live2d_model might be None
        model_info = None
        if context.live2d_model and hasattr(context.live2d_model, 'model_info'):
            model_info = context.live2d_model.model_info
        
        if not model_info:
            logger.error(f"[WebSocket] Live2D model not initialized for client {client_uid}")
            logger.error(f"[WebSocket] live2d_model: {context.live2d_model}")
            logger.error(f"[WebSocket] live2d_model_name: {context.character_config.live2d_model_name if context.character_config else 'N/A'}")
            # Don't send None, send empty dict to prevent frontend errors
            model_info = {}
