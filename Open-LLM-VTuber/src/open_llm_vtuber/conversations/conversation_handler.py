...
from .single_conversation import process_single_conversation, process_speak_direct
...
async def handle_conversation_trigger(
    ...
    websocket_send_override: Optional[Callable] = None,  # NEW: optional override for broadcasting
) -> None:
    ...
    if msg_type == "speak":
        ...
        logger.info(f"[SpeakDirect] Received speak command (bypassing brain): {text[:50]}...")

        # Use broadcast function if provided, otherwise use websocket.send_text
        ...
        current_conversation_tasks[client_uid] = asyncio.create_task(
            process_speak_direct(
                context=context,
                websocket_send=send_func,  # Use broadcast or direct send
