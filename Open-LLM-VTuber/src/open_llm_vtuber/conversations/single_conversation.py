...
async def process_speak_direct(
    context: ServiceContext,
    websocket_send: WebSocketSend,
    client_uid: str,
    text: str,
    ...
    """
    [Mellow-Fix] 입마개 해제 버전.
    브라우저에게 '오디오 준비 끝, 재생해!' 신호를 확실하게 보냅니다.
    """
    tts_manager = TTSTaskManager()
    ...
    try:
        ...
        if tts_manager.task_list:
            ...
            # [CRITICAL] payload 큐가 비워질 때까지 대기 (실제 WebSocket 전송 완료)
            await tts_manager._payload_queue.join()

            # [핵심] 이 로그가 찍혀야 해! "silent bypass"가 아니라!
            logger.debug("[SpeakDirect] Sending backend-synth-complete signal...")
            await websocket_send(json.dumps({"type": "backend-synth-complete"}))
        
        logger.debug(f"[SpeakDirect] Completed: {text[:50]}...")
        
    except Exception as e:
        logger.error(f"[SpeakDirect] Error: {e}")
        await websocket_send(json.dumps({"type": "error", "message": f"TTS Error: {str(e)}"}))
    finally:
        cleanup_conversation(tts_manager, session_emoji)
