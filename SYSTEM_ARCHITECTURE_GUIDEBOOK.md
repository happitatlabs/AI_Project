...
| **Mellow-Link** | Orchestration, auth, sessions, relay | No TTS, no avatar rendering |
...
│   ├── vtuber_relay.py       # WebSocket relay to VTuber
│   └── image_service.py      # ComfyUI client
├── infra\
│   └── database.py           # SQLite models, auth functions
├── core\
│   ├── orchestrator.py       # GPU state machine
...
## 2.4 Language Guardrail Configuration
...
**File:** `mellow_link/main.py:1574-1588`

```python
# [CRITICAL FIX] Force System Prompt to be applied properly
mandatory_base_prompt = (
    "반드시 한국어로만 답하세요. "
    "답변은 무조건 1-2문장으로 짧게 하세요. "
    "긴 설명, 리스트, 번호 매기기 금지. "
    "영어 사용 금지."
)

# Always prepend the mandatory prompt
if system_prompt and system_prompt.strip():
    system_prompt = f"{mandatory_base_prompt}\n\nAdditional context: {system_prompt}"
else:
    system_prompt = mandatory_base_prompt
```
...
```python
# Remove any Chinese characters that might have leaked
import re
...
```python
# When session is loaded, restore LLM context from database
if session:
...
**File:** `mellow_link/main.py:324-374`
