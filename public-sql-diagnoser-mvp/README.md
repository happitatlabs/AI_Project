...
- SQL 입력 및 즉시 진단
- 변경 전후 SQL 구조 비교와 검토 체크리스트
- 단건 SQL 진단 / 다건 SQL 자산 지도
- 테이블 중심 자산 지도 생성
- 시스템 의존성 지도와 영향도 분석
- 보고서 / 문서화 산출물 생성
- 리스크 / 개선 포인트 탐지
...
- WHERE / HAVING 범위 조건
- GROUP BY 기준과 집계 지표
- CTE, 서브쿼리, 윈도우 함수, SET 연산
- 변경 후 새로 감지된 룰 기반 리스크

결과는 `핵심 변경 → 그래서 무엇이 중요한가 → 다음 질문 → 변경 전 체크리스트 → 상세 근거` 순서로 표시됩니다. SQL을 실행하지 않으므로 실제 행 수, 실행 계획, 제약조건은 확인 질문으로 남기며 성능이나 데이터 영향을 확정하지 않습니다.

화면에는 JOIN·집계 변경, UPDATE 조건 제거, 쓰기 대상 변경, `SELECT *` 도입, 구조 동일 대조군의 5개 대표 검증 사례가 포함됩니다. 각 사례의 기대 심각도와 필수 finding·질문은 자동 테스트로 고정합니다.
...
CSV / JSON 원본 행 데이터
-> 브라우저 내 기계식 계산
-> 검증 가능한 계산 결과와 보고서용 핵심 결과
-> 선택 실행 AI 해석
-> 사실 / 해석 / 확인 필요 사항 / 제안 보고서
```

입력 직후 프로그램은 컬럼 이름과 값의 형태를 기준으로 시간·수치 지표·범주·식별자 가능성을 추론하고, 추천 분석 질문을 먼저 제시합니다. 기본 추천 기준으로 바로 실행할 수 있으며, 자동 감지가 데이터 의미와 다를 때만 접힌 `분석 기준 직접 수정` 영역에서 시간/지표/비교 기준을 바꿉니다.
...
- 시간×그룹별 변화량과 기간 순변화 대비 기여도
- 전체·합계 행 감지 후 비교/이상치 계산에서의 중복 집계 방지
- IQR 또는 Z-score 기반 통계적 추가 확인 값
- 변화폭, 지속성, 그룹 차이, 추가 확인 필요 여부를 근거로 한 보고서용 핵심 결과

시간과 그룹이 모두 있는 경우에는 `관찰 → 변화가 발생한 그룹 분해 → 다음 분석 질문` 흐름을 만들고, 다른 범주 컬럼이 있으면 세부 분해 질문을 제안합니다. 다만 현재 드릴다운은 자동으로 모든 조합을 계산하지 않으며, 사용자가 제안된 다른 비교 기준으로 다시 실행하는 방식입니다.

변화폭·지속성·그룹 차이·통계적 추가 확인 여부에 대한 내부 우선순위는 AI가 해석할 항목을 고르기 위한 계산 정보입니다. 화면과 Markdown 보고서의 기본 영역에는 점수나 후보 목록을 노출하지 않고, 자료 작성에 바로 쓸 수 있는 `보고서용 핵심 결과`를 먼저 표시합니다. 내부 계산 근거는 `계산 검증 상세`를 펼쳤을 때만 확인할 수 있습니다.

`ComputedAnalysisResult`에는 계약 버전과 계산 기준도 포함됩니다. 계산 기준에는 자동 추론한 분석 계획, 입력/계산/제외 행 수, 전체·합계 행 분리 여부, 기간 범위와 단위, 그룹 비교 기준, 기간×그룹 변화 분해 범위, IQR·Z-score 임계값, 반복 주기 미평가 여부가 기록됩니다. 따라서 AI는 계산 방법을 바꾸거나, 반복 주기·계절성처럼 계산하지 않은 내용을 단정할 수 없습니다.

AI는 원본 행 데이터를 받지 않습니다. `/api/ai-data-insights`에는 원본 행이 없는 `ComputedAnalysisResult`만 전달되고, 서버는 `rows` 필드 또는 유효하지 않은 계산 계약을 포함한 요청을 거부합니다. AI provider로는 마스킹된 범주 정보, 계산된 수치, 변화 기여도, 근거 ID, 계산 기준, 내부 선택 우선순위만 전송합니다. AI는 보고서용 핵심 결과에 해석·확인 필요 사항·제안만 보강하며, 표시되는 사실과 숫자는 프로그램이 계산한 결과를 사용합니다.

데이터 분석 Markdown 보고서는 다음 순서로 생성됩니다.

- 분석 개요와 데이터 범위
- 핵심 지표와 최대 3개의 주요 결과
- 선택 실행 AI 해석·확인 필요 사항·제안
- 결론과 주의 사항
- 계산 기준과 검증 범위

입력 데이터는 최대 20,000행까지 계산합니다. 현재 시간 추세는 `YYYY`, `YYYY-MM`, `YYYY-MM-DD` 등 일반적인 연도/월/날짜 형식에 한정됩니다. 전체·합계 표기 감지는 선택한 그룹 컬럼 기준의 안전장치이며, 다단계 집계 수준을 완전한 통계 모델로 판별하지는 않습니다. 중첩 JSON, 복잡한 날짜 형식, 통계적 인과 검증은 지원 범위가 아닙니다.
