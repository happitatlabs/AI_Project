...
GOAL_PATH = os.path.join(BASE_DIR, "agent_goal.md")
...
def adapter() -> LLMAdapter:
    adapter = LLMAdapter.from_config(CONFIG_PATH)
    if not adapter.is_available():
        pytest.skip(f"Ollama unavailable: {adapter.get_last_error()}")
    return adapter


# ── T1: Ping ─────────────────────────────────────────
def _run_ping(adapter: LLMAdapter) -> bool:
...
def test_ping(adapter: LLMAdapter) -> None:
    assert _run_ping(adapter)


# ── T2: 모델 목록 ─────────────────────────────────────
def _run_model_list(adapter: LLMAdapter) -> bool:
...
def test_model_list(adapter: LLMAdapter) -> None:
    assert _run_model_list(adapter)


# ── T3: 단순 chat ─────────────────────────────────────
def _run_simple_chat(adapter: LLMAdapter) -> bool:
...
def test_simple_chat(adapter: LLMAdapter) -> None:
    assert _run_simple_chat(adapter)


# ── T4: JSON 응답 파싱 ────────────────────────────────
def _run_json_response(adapter: LLMAdapter) -> bool:
...
def test_json_response(adapter: LLMAdapter) -> None:
    assert _run_json_response(adapter)


# ── T5: Planner.analyze_gap LLM ──────────────────────
def _run_planner_gap(adapter: LLMAdapter) -> bool:
    section("T5: Planner.analyze_gap (LLM 경로)")
    planner = Planner(goal_file=GOAL_PATH, llm=adapter)
...
def test_planner_gap(adapter: LLMAdapter) -> None:
    assert _run_planner_gap(adapter)


# ── T6: Planner.pick_next_action LLM ─────────────────
def _run_planner_pick(adapter: LLMAdapter) -> bool:
...
def test_planner_pick(adapter: LLMAdapter) -> None:
    assert _run_planner_pick(adapter)
...
def main():
    ...
    # T1은 항상 실행
    results["T1_ping"] = _run_ping(adapter)
    ...
    results["T2_models"]      = _run_model_list(adapter)
    results["T3_chat"]        = _run_simple_chat(adapter)
    results["T4_json"]        = _run_json_response(adapter)
    results["T5_planner_gap"] = _run_planner_gap(adapter)
    results["T6_planner_pick"]= _run_planner_pick(adapter)
