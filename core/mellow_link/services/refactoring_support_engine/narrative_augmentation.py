...
class NarrativeAugmentationService:
    ...
    def apply(
        ...
        if isinstance(existing_narrative, dict):
            narrative_axis = str(existing_narrative.get("axis") or "").strip()
        if not narrative_axis:
            narrative_axis = str(result.narrative_axis or result.template_judgment or result.primary_judgment or "").strip()
    ...
    def _build_request(self, *, prepared, result: StructuredRebuildResult) -> NarrativeAugmentationRequest:
        ...
        return NarrativeAugmentationRequest(
            ...
            primary_judgment=result.template_judgment or result.primary_judgment,
            narrative_axis=str(narrative_extension.get("axis") or result.narrative_axis or result.template_judgment or result.primary_judgment or ""),
