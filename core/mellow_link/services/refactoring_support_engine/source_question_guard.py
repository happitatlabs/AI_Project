...
_MISMATCH_PATTERNS: tuple[tuple[str, tuple[str, ...]], ...] = (
    (
        "product_validation",
        ("product", "제품 저장", "저장 전 검증", "입력 검증 로직", "save validation"),
    ),
    (
        "sql_parameter",
        (
...
class SourceQuestionGuardService:
    ...
    def evaluate(
        ...
        *,
        ...
        if (
            blocks
            and source_char_count >= self._FALLBACK_MIN_SOURCE_CHARS
            and not source_domain_terms
        ):
        ...
        if (
            ...
            and blocks
            and all(
                block.get("source_stage") not in {"sml", "document"} for block in blocks
            )
        ):
        ...
        for item in self._user_questions(
            raw_goal=raw_goal, raw_constraints=raw_constraints
        ):
        ...
        selected_questions = self._selected_questions(
            candidates, allowed_user_questions
        )
        preferred_question_axis = self._preferred_question_axis(
            source_text=source_text, candidates=candidates
        )
        if not selected_questions:
            selected_questions = [
                "입력 소스에서 확인 가능한 문제와 개선 방향은 무엇인가?"
            ]
        ...
        summary = QuestionGuardSummary(
            ...
            selected_question_types=self._selected_question_types(
                candidates, selected_questions
            ),
            source_domain_terms=source_domain_terms,
    ...
    def _analysis_blocks(
        self, analysis_context: AnalysisContextBundle
    ) -> list[dict[str, str]]:
        ...
        for source in list(analysis_context.source_blocks or []):
            ...
            if not content:
                continue
            asset_name = str(
                source.asset_name or source.locator or source.asset_id or ""
            ).strip()
            source_stage = "document"
            lowered = content.lower()
            asset_type = str(source.asset_type or "").strip().lower()
            if lowered.startswith("[sml v1]") or asset_name.lower().endswith(
                (".ppt", ".pptx")
            ):
                source_stage = "sml"
    ...
    def _source_domain_terms(self, source_text: str) -> list[str]:
        ...
        if not self._has_costing_profile(lowered):
            return []
        found = [term for term in _COST_KEYWORDS if term.lower() in lowered]
        return found[:8]

    def _has_costing_profile(self, lowered_source_text: str) -> bool:
        anchor_hits = sum(
            1 for term in _COSTING_ANCHOR_TERMS if term.lower() in lowered_source_text
        )
        has_full_cost_components = all(
            term in lowered_source_text for term in ("재료비", "노무비", "제조경비")
        )
        has_allocation_detail = (
            "배부" in lowered_source_text or "배부기준" in lowered_source_text
        )
        # A stray "원가" or "손익" mention appears in many generic consulting decks.
        # Only switch to costing-specific questions when the source has both a
        # costing anchor and concrete costing components. Generic ISP decks can
        # contain isolated material/labor/cost references without being a
        # costing-consulting deck.
        return anchor_hits >= 1 and has_full_cost_components and has_allocation_detail
    ...
    def _extract_candidates(
        ...
        if len(source_domain_terms) >= 2:
            ...
            if all(term in lowered for term in ("재료비", "노무비", "제조경비")) and (
                "배부" in lowered or "배부기준" in lowered
            ):
                candidates.append(
                    self._make_candidate(
    ...
    def _generic_fallback_candidates(
    ...
    def _make_candidate(
        ...
        *,
        ...
        asset_id, stage, snippet = self._find_snippet(
            blocks, keywords, default_stage=default_stage
        )
    ...
    def _find_snippet(
        ...
        *,
        ...
        for block in blocks:
            ...
            for raw_line in content.splitlines():
                ...
                if any(keyword in lowered for keyword in normalized_keywords):
                    return (
                        block["asset_id"] or None,
                        block.get("source_stage") or default_stage,
                        line[:220],
                    )
        if not blocks:
        ...
        first = blocks[0]
        fallback_line = (
            re.sub(r"\s+", " ", str(first["content"]).splitlines()[0]).strip()[:220]
            if str(first["content"]).splitlines()
            else ""
        )
        return (
            first["asset_id"] or None,
            first.get("source_stage") or default_stage,
            fallback_line,
        )
    ...
    def _guard_user_question(
        ...
        *,
        ...
        if self._is_source_domain_mismatch(
            lowered, source_domain_terms=source_domain_terms
        ):
            return GuardedUserQuestion(
                question=normalized,
                status="blocked",
                blocked_reason="source_domain_mismatch",
                matched_source_question=(
                    matched_candidate.question if matched_candidate else ""
                ),
                evidence_snippet=(
                    matched_candidate.evidence_snippet if matched_candidate else ""
                ),
            )
        ...
        if source_domain_terms and any(
            term.lower() in lowered for term in source_domain_terms
        ):
            return GuardedUserQuestion(question=normalized, status="allowed")
        if (
            normalized.endswith("?")
            or normalized.endswith("가")
            or normalized.endswith("까")
        ):
            return GuardedUserQuestion(
                question=normalized,
                status="needs_review",
                blocked_reason="weak_source_grounding",
            )
        return GuardedUserQuestion(question=normalized, status="allowed")
    ...
    def _is_source_domain_mismatch(
        self, lowered_question: str, *, source_domain_terms: list[str]
    ) -> bool:
    ...
    def _is_conclusion_forcing(self, lowered_question: str) -> bool:
        return any(
            re.search(pattern, lowered_question, flags=re.IGNORECASE)
            for pattern in _FORCING_PATTERNS
        )
    ...
    def _selected_question_types(
        ...
        question_map = {
            candidate.question: candidate.question_type for candidate in candidates
        }
    ...
    def _preferred_question_axis(
        ...
        *,
        ...
        if calc_hits >= 3 and any(
            candidate.question_type == "decision_criteria" for candidate in candidates
        ):
    ...
    def _dedupe_candidates(
        self, candidates: list[SourceQuestionCandidate]
    ) -> list[SourceQuestionCandidate]:
