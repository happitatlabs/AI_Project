    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    pytest.skip(f"SSE endpoint unavailable: HTTP {resp.status}")
                ...
                if done_metadata:
                    ...
                    pytest.fail("No done event received")
    except aiohttp.ClientError as exc:
        pytest.skip(f"Local server unavailable: {exc}")


def test_sse_metadata():
    asyncio.run(_run_sse_metadata())
