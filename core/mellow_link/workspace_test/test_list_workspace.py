...
def test_tree_mode() -> None:
    assert _run_tree_mode()


def _run_flat_mode() -> bool:
    """플랫 모드 테스트"""
    print("\n=== 플랫 모드 테스트 ===")
    result = subprocess.run(
...
def test_flat_mode() -> None:
    assert _run_flat_mode()


def _run_relative_mode() -> bool:
    """상대 경로 모드 테스트"""
    print("\n=== 상대 경로 모드 테스트 ===")
    result = subprocess.run(
...
def test_relative_mode() -> None:
    assert _run_relative_mode()
...
def main():
    ...
    tests = [
        ("트리 모드", _run_tree_mode),
        ("플랫 모드", _run_flat_mode),
        ("상대 경로 모드", _run_relative_mode),
    ]
