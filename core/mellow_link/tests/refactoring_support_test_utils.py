...
def load_expansion_sample_case(sample_name: str) -> dict[str, object]:
    sample_dir = SAMPLES_ROOT / sample_name
    manifest = json.loads((sample_dir / "input_manifest.json").read_text(encoding="utf-8"))
    asset_specs = []
    for asset in manifest["assets"]:
        asset_path_value = str(asset["path"]).replace("\\", "/")
        if asset_path_value.startswith("notes/"):
            continue
        asset_path = sample_dir / asset["path"]
        asset_specs.append({"name": asset["name"], "content": asset_path.read_text(encoding="utf-8")})
    return {
        "sample_dir": sample_dir,
        "goal": manifest.get("title") or manifest.get("description") or sample_name,
        "constraints": [],
        "manifest": manifest,
        "asset_specs": asset_specs,
        "safe_bundle": build_safe_bundle(asset_specs),
...
def load_expected_assertions(sample_name: str) -> dict[str, object]:
    sample_dir = SAMPLES_ROOT / sample_name
    return yaml.safe_load((sample_dir / "expected_assertions.yaml").read_text(encoding="utf-8"))
