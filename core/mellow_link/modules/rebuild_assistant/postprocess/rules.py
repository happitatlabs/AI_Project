...
def collect_pattern_warnings(pattern_axis: str, text: str) -> list[str]:
    warnings: list[str] = []
    for phrase in PATTERN_FORBIDDEN_EXPRESSIONS.get(pattern_axis, []):
        if phrase and phrase in (text or ""):
            warnings.append(f"{pattern_axis} 문서에 금지 표현이 남아 있습니다: {phrase}")
    return warnings
